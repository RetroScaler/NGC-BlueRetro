                                                              ...
                                                              struct lfds711_list_asu_state *list_of_gnuplot_strings )
{
  ...
  LFDS711_PAL_ASSERT( list_of_gnuplot_strings != NULL );

  lfds711_list_asu_init_valid_on_current_logical_core( list_of_gnuplot_strings, NULL );
  ...
  while( LFDS711_LIST_ASU_GET_START_AND_THEN_NEXT(bss->benchmarksets,lasue) )
  {
    ...
    while( LFDS711_LIST_ASU_GET_START_AND_THEN_NEXT(*bsets->numa_modes_list,lasue_numa) )
    {
      ...
      bg = libshared_memory_alloc_from_most_free_space_node( bss->ms, sizeof(struct libbenchmark_benchmarkset_gnuplot), LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );

      libbenchmark_benchmarkset_gnuplot_emit( bsets, rs, gnuplot_system_string, numa_mode->mode, gpo, bg );

      LFDS711_LIST_ASU_SET_VALUE_IN_ELEMENT( bg->lasue, bg );
      lfds711_list_asu_insert_at_end( list_of_gnuplot_strings, &bg->lasue );
    }
  }
