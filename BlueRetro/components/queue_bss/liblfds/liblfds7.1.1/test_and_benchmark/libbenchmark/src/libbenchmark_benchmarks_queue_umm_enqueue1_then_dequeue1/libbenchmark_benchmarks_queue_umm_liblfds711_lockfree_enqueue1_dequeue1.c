...
/***** structs *****/
struct libbenchmark_benchmark_queue_umm_liblfds711_lockfree_enqueue1_dequeue1_per_thread_benchmark_state
{
  lfds711_pal_uint_t
    operation_count;
};

struct libbenchmark_benchmark_queue_umm_liblfds711_lockfree_enqueue1_dequeue1_overall_benchmark_state
{
...
/****************************************************************************/
void libbenchmark_benchmark_queue_umm_liblfds711_lockfree_enqueue1_dequeue1_init( struct libbenchmark_topology_state *ts,
                                                                               ...
                                                                               struct libbenchmark_threadset_state *tsets )
{
  ...
  obs = libshared_memory_alloc_from_most_free_space_node( ms, sizeof(struct libbenchmark_benchmark_queue_umm_liblfds711_lockfree_enqueue1_dequeue1_overall_benchmark_state), LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );

  libbenchmark_threadset_init( tsets, ts, logical_processor_set, ms, libbenchmark_benchmark_queue_umm_liblfds711_lockfree_enqueue1_dequeue1_thread, NULL );

  switch( numa_mode )
  {
    case LIBBENCHMARK_TOPOLOGY_NUMA_MODE_SMP:
      qs = libshared_memory_alloc_from_most_free_space_node( ms, sizeof(struct lfds711_queue_umm_state), LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );
      ...
      qe = libshared_memory_alloc_from_most_free_space_node( ms, sizeof(struct lfds711_queue_umm_element) * (number_logical_processors+1), LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );
      lfds711_queue_umm_init_valid_on_current_logical_core( qs, &qe[0], NULL );
      for( loop = 1 ; loop < (number_logical_processors+1) ; loop++ )
        lfds711_queue_umm_enqueue( qs, &qe[loop] );
      ...
      while( LFDS711_LIST_ASU_GET_START_AND_THEN_NEXT(tsets->list_of_per_thread_states,lasue) )
      {
        ...
        ptbs = libshared_memory_alloc_from_most_free_space_node( ms, sizeof(struct libbenchmark_benchmark_queue_umm_liblfds711_lockfree_enqueue1_dequeue1_per_thread_benchmark_state), LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );
    ...
    case LIBBENCHMARK_TOPOLOGY_NUMA_MODE_NUMA:
      ...
      qs = libshared_memory_alloc_from_specific_node( ms, largest_pns->numa_node_id, sizeof(struct lfds711_queue_umm_state), LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );
      qe = libshared_memory_alloc_from_specific_node( ms, largest_pns->numa_node_id, sizeof(struct lfds711_queue_umm_element), LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );
      lfds711_queue_umm_init_valid_on_current_logical_core( qs, qe, NULL );
      ...
      */
      ...
      while( LFDS711_LIST_ASU_GET_START_AND_THEN_NEXT(tsets->list_of_per_numa_states,lasue) )
      {
        ...
        */
        ...
        qe = libshared_memory_alloc_from_specific_node( ms, pns->numa_node_id, sizeof(struct lfds711_queue_umm_element) * number_logical_processors_in_numa_node, LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );
      ...
      {
        pts = LFDS711_LIST_ASU_GET_VALUE_FROM_ELEMENT( *lasue );
        ptbs = libshared_memory_alloc_from_specific_node( ms, largest_pns->numa_node_id, sizeof(struct libbenchmark_benchmark_queue_umm_liblfds711_lockfree_enqueue1_dequeue1_per_thread_benchmark_state), LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );
        pts->users_per_thread_state = ptbs;
      }
    break;
      ...
      */
      ...
      while( LFDS711_LIST_ASU_GET_START_AND_THEN_NEXT(tsets->list_of_per_numa_states,lasue) )
      {
        ...
        */
        ...
        qe = libshared_memory_alloc_from_specific_node( ms, largest_pns->numa_node_id, sizeof(struct lfds711_queue_umm_element) * number_logical_processors_in_numa_node, LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );
        for( loop = 0 ; loop < number_logical_processors_in_numa_node ; loop++ )
          lfds711_queue_umm_enqueue( qs, &qe[loop] );
      }
...
/****************************************************************************/
libshared_pal_thread_return_t LIBSHARED_PAL_THREAD_CALLING_CONVENTION libbenchmark_benchmark_queue_umm_liblfds711_lockfree_enqueue1_dequeue1_thread( void *libbenchmark_threadset_per_thread_state )
{
  ...
  {
    lfds711_queue_umm_dequeue( qs, &qe );
...
/****************************************************************************/
void libbenchmark_benchmark_queue_umm_liblfds711_lockfree_enqueue1_dequeue1_cleanup( struct lfds711_list_aso_state *logical_processor_set,
                                                                                  enum libbenchmark_topology_numa_mode numa_mode,
                                                                                  ...
                                                                                  struct libbenchmark_threadset_state *tsets )
{
  ...
  lfds711_queue_umm_cleanup( obs->qs, NULL );
