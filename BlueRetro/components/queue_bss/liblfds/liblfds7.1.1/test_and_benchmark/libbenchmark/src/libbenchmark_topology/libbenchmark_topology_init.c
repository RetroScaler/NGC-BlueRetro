...
int libbenchmark_topology_init( struct libbenchmark_topology_state *ts, struct libshared_memory_state *ms )
{
  ...
  lfds711_btree_au_init_valid_on_current_logical_core( &ts->topology_tree, libbenchmark_topology_node_compare_nodes_function, LFDS711_BTREE_AU_EXISTING_KEY_FAIL, NULL );

  rv = libbenchmark_porting_abstraction_layer_populate_topology( ts, ms );

  lfds711_btree_au_get_by_absolute_position( &ts->topology_tree, &baue, LFDS711_BTREE_AU_ABSOLUTE_POSITION_LARGEST_IN_TREE );
  ...
  lfds711_btree_au_init_valid_on_current_logical_core( &ts->lp_printing_offset_lookup_tree, libbenchmark_topology_compare_lp_printing_offsets_function, LFDS711_BTREE_AU_EXISTING_KEY_FAIL, NULL );
  ...
  while( lfds711_btree_au_get_by_absolute_position_and_then_by_relative_position(&ts->topology_tree, &baue, LFDS711_BTREE_AU_ABSOLUTE_POSITION_LARGEST_IN_TREE, LFDS711_BTREE_AU_RELATIVE_POSITION_NEXT_SMALLER_ELEMENT_IN_ENTIRE_TREE) )
  {
    ...
    {
      tlpo = libshared_memory_alloc_from_most_free_space_node( ms, sizeof(struct libbenchmark_topology_lp_printing_offset), LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );

      tlpo->tns = *tns;
      tlpo->offset = offset;

      LFDS711_BTREE_AU_SET_KEY_IN_ELEMENT( tlpo->baue, tlpo );
      LFDS711_BTREE_AU_SET_VALUE_IN_ELEMENT( tlpo->baue, tlpo );

      lfds711_btree_au_insert( &ts->lp_printing_offset_lookup_tree, &tlpo->baue, NULL );
