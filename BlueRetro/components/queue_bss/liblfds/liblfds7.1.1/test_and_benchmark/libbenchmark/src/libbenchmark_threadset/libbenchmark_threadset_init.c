                                  ...
                                  void *users_threadset_state )
{
  ...
  lfds711_list_asu_init_valid_on_current_logical_core( &tsets->list_of_per_numa_states, NULL );
  lfds711_list_asu_init_valid_on_current_logical_core( &tsets->list_of_per_thread_states, NULL );
  ...
  */
  ...
  while( LFDS711_LIST_ASO_GET_START_AND_THEN_NEXT(*logical_processor_set,lasoe) )
  {
    ...
    if( tns_numa_node != NULL )
    {
      ...
      {
        pns = LFDS711_LIST_ASO_GET_VALUE_FROM_ELEMENT( *lasue );
      ...
      if( lasue == NULL )
      {
        pns = libshared_memory_alloc_from_specific_node( ms, LIBBENCHMARK_TOPOLOGY_NODE_GET_NUMA_ID(*tns_numa_node), sizeof(struct libbenchmark_threadset_per_numa_state), LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );

        pns->numa_node_id = LIBBENCHMARK_TOPOLOGY_NODE_GET_NUMA_ID( *tns_numa_node );
        pns->users_per_numa_state = NULL;
        LFDS711_LIST_ASU_SET_VALUE_IN_ELEMENT( pns->lasue, pns );
        lfds711_list_asu_insert_at_start( &tsets->list_of_per_numa_states, &pns->lasue );
      }
    }

    // TRD : now make a thread entry (pns now points at the correct NUMA node entry)
    if( pns == NULL )
      pts = libshared_memory_alloc_from_most_free_space_node( ms, sizeof(struct libbenchmark_threadset_per_thread_state), LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );
    else
      pts = libshared_memory_alloc_from_specific_node( ms, pns->numa_node_id, sizeof(struct libbenchmark_threadset_per_thread_state), LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );
    ...
    lfds711_list_asu_insert_at_start( &tsets->list_of_per_thread_states, &pts->lasue );
  }
