...
/***** structs *****/
struct libbenchmark_benchmark_freelist_liblfds711_lockfree_push1_pop1_per_thread_benchmark_state
{
  lfds711_pal_uint_t
    operation_count;
...
struct libbenchmark_benchmark_freelist_liblfds711_lockfree_push1_pop1_overall_benchmark_state
{
...
/****************************************************************************/
void libbenchmark_benchmark_freelist_liblfds711_lockfree_push1_pop1_init( struct libbenchmark_topology_state *ts,
                                                                       ...
                                                                       struct libbenchmark_threadset_state *tsets )
{
  ...
  lfds711_pal_uint_t
    ...
    *fe_array_count,
    ...
    number_numa_nodes,
    number_freelist_element_pointers_per_atomic_isolation = LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES / sizeof(struct lfds711_freelist_element *),
  ...
  obs = libshared_memory_alloc_from_most_free_space_node( ms, sizeof(struct libbenchmark_benchmark_freelist_liblfds711_lockfree_push1_pop1_overall_benchmark_state), LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );

  libbenchmark_threadset_init( tsets, ts, logical_processor_set, ms, libbenchmark_benchmark_freelist_liblfds711_lockfree_push1_pop1_thread, NULL );
  ...
  switch( numa_mode )
  {
    case LIBBENCHMARK_TOPOLOGY_NUMA_MODE_SMP:
      ...
      fs = libshared_memory_alloc_from_most_free_space_node( ms, sizeof(struct lfds711_freelist_state), LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );
      ...
      fe = libshared_memory_alloc_from_most_free_space_node( ms, sizeof(struct lfds711_freelist_element) * number_freelist_elements, LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );
      for( loop = 0 ; loop < number_freelist_elements ; loop++ )
        lfds711_freelist_push( fs, &fe[loop], &psts );
      while( LFDS711_LIST_ASU_GET_START_AND_THEN_NEXT(tsets->list_of_per_thread_states,lasue) )
      {
        ...
        ptbs = libshared_memory_alloc_from_most_free_space_node( ms, sizeof(struct libbenchmark_benchmark_freelist_liblfds711_lockfree_push1_pop1_per_thread_benchmark_state), LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );
        LFDS711_PRNG_ST_GENERATE( psts, random_value );
        LFDS711_PRNG_ST_MIXING_FUNCTION( random_value );
        lfds711_prng_st_init( &ptbs->psts, random_value );
    ...
    case LIBBENCHMARK_TOPOLOGY_NUMA_MODE_NUMA:
      ...
      */
      ...
      fs = libshared_memory_alloc_from_specific_node( ms, largest_pns->numa_node_id, sizeof(struct lfds711_freelist_state), LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );
      ea = libshared_memory_alloc_from_specific_node( ms, largest_pns->numa_node_id, sizeof(struct lfds711_freelist_element *) * LFDS711_FREELIST_ELIMINATION_ARRAY_ELEMENT_SIZE_IN_FREELIST_ELEMENTS * smallest_power_of_two_larger_than_or_equal_to_number_logical_processors, LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );
      lfds711_freelist_init_valid_on_current_logical_core( fs, ea, smallest_power_of_two_larger_than_or_equal_to_number_logical_processors, NULL );
      ...
      */
      ...
      libbenchmark_topology_query( ts, LIBBENCHMARK_TOPOLOGY_QUERY_GET_NUMBER_OF_NODE_TYPE, (void *) (lfds711_pal_uint_t) LIBBENCHMARK_TOPOLOGY_NODE_TYPE_NUMA, (void *) &number_numa_nodes );

      fe_array_pointers = libshared_memory_alloc_from_most_free_space_node( ms, sizeof(struct lfds711_freelist_element *) * number_numa_nodes, LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );
      fe_array_count = libshared_memory_alloc_from_most_free_space_node( ms, sizeof(lfds711_pal_uint_t) * number_numa_nodes, LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );
      for( loop = 0 ; loop < number_numa_nodes ; loop++ )
        fe_array_count[loop] = 0;

      // TRD : now query the freelist for the EL size
      lfds711_freelist_query( fs, LFDS711_FREELIST_QUERY_GET_ELIMINATION_ARRAY_EXTRA_ELEMENTS_IN_FREELIST_ELEMENTS, NULL, (void *) &ea_size_in_freelist_elements );
      ...
      while( LFDS711_LIST_ASU_GET_START_AND_THEN_NEXT(tsets->list_of_per_numa_states,lasue) )
      {
        ...
        */
        ...
        fe_array_pointers[index] = libshared_memory_alloc_from_specific_node( ms, pns->numa_node_id, sizeof(struct lfds711_freelist_element) * fe_array_count[index], LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );
        index++;
      }
      ...
      while( finished_flag == LOWERED )
      {
        for( loop = 0 ; loop < index ; loop++ )
          if( fe_array_count[loop] > 0 )
            lfds711_freelist_push( fs, &fe_array_pointers[loop][ fe_array_count[loop]-- ], &psts );
      ...
      {
        pts = LFDS711_LIST_ASU_GET_VALUE_FROM_ELEMENT( *lasue );
        ptbs = libshared_memory_alloc_from_specific_node( ms, largest_pns->numa_node_id, sizeof(struct libbenchmark_benchmark_freelist_liblfds711_lockfree_push1_pop1_per_thread_benchmark_state), LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );
        LFDS711_PRNG_ST_GENERATE( psts, random_value );
        LFDS711_PRNG_ST_MIXING_FUNCTION( random_value );
        lfds711_prng_st_init( &ptbs->psts, random_value );
        pts->users_per_thread_state = ptbs;
    ...
    case LIBBENCHMARK_TOPOLOGY_NUMA_MODE_NUMA_BUT_NOT_USED:
      ...
      */
      ...
      // TRD : fill the elimination array and have one element per thread in the freelist proper
      number_freelist_elements = (smallest_power_of_two_larger_than_or_equal_to_number_logical_processors * number_freelist_element_pointers_per_atomic_isolation) + number_logical_processors;
      fe = libshared_memory_alloc_from_specific_node( ms, largest_pns->numa_node_id, sizeof(struct lfds711_freelist_element) * number_freelist_elements, LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );
      for( loop = 0 ; loop < number_freelist_elements ; loop++ )
        lfds711_freelist_push( fs, &fe[loop], &psts );
...
/****************************************************************************/
libshared_pal_thread_return_t LIBSHARED_PAL_THREAD_CALLING_CONVENTION libbenchmark_benchmark_freelist_liblfds711_lockfree_push1_pop1_thread( void *libbenchmark_threadset_per_thread_state )
{
  ...
  {
    lfds711_freelist_pop( fs, &fe, &ptbs->psts );
    lfds711_freelist_push( fs, fe, &ptbs->psts );
    operation_count++;
...
/****************************************************************************/
void libbenchmark_benchmark_freelist_liblfds711_lockfree_push1_pop1_cleanup( struct lfds711_list_aso_state *logical_processor_set,
                                                                          enum libbenchmark_topology_numa_mode numa_mode,
                                                                          ...
                                                                          struct libbenchmark_threadset_state *tsets )
{
  ...
  lfds711_freelist_cleanup( obs->fs, NULL );
