...
int libbenchmark_topology_iterate( struct libbenchmark_topology_state *ts, struct libbenchmark_topology_iterate_state *tis, struct libbenchmark_topology_node_state **tns )
{
  ...
  while( lfds711_btree_au_get_by_absolute_position_and_then_by_relative_position(&ts->topology_tree, &tis->baue, LFDS711_BTREE_AU_ABSOLUTE_POSITION_LARGEST_IN_TREE, LFDS711_BTREE_AU_RELATIVE_POSITION_NEXT_SMALLER_ELEMENT_IN_ENTIRE_TREE) )
  {
    *tns = LFDS711_BTREE_AU_GET_VALUE_FROM_ELEMENT( *tis->baue );
