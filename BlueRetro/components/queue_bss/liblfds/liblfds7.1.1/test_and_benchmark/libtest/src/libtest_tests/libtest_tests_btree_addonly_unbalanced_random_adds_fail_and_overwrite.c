...
void libtest_tests_btree_au_fail_and_overwrite_on_existing_key( struct lfds711_list_asu_state *list_of_logical_processors, struct libshared_memory_state *ms, enum lfds711_misc_validity *dvs )
{
  ...
  */
  ...
  LFDS711_BTREE_AU_SET_KEY_IN_ELEMENT( baue_one, 0 );
  LFDS711_BTREE_AU_SET_VALUE_IN_ELEMENT( baue_one, 1 );
  alr = lfds711_btree_au_insert( &baus, &baue_one, NULL );

  if( alr != LFDS711_BTREE_AU_INSERT_RESULT_SUCCESS )
    *dvs = LFDS711_MISC_VALIDITY_INVALID_TEST_DATA;

  LFDS711_BTREE_AU_SET_KEY_IN_ELEMENT( baue_two, 0 );
  LFDS711_BTREE_AU_SET_VALUE_IN_ELEMENT( baue_two, 2 );
  alr = lfds711_btree_au_insert( &baus, &baue_two, &existing_baue );

  if( alr != LFDS711_BTREE_AU_INSERT_RESULT_FAILURE_EXISTING_KEY )
    *dvs = LFDS711_MISC_VALIDITY_INVALID_TEST_DATA;

  if( existing_baue != &baue_one )
    *dvs = LFDS711_MISC_VALIDITY_INVALID_TEST_DATA;

  value = LFDS711_BTREE_AU_GET_VALUE_FROM_ELEMENT( *existing_baue );

  if( (void *) (lfds711_pal_uint_t) value != (void *) (lfds711_pal_uint_t) 1 )
    *dvs = LFDS711_MISC_VALIDITY_INVALID_TEST_DATA;
  ...
  */
  ...
  LFDS711_BTREE_AU_SET_KEY_IN_ELEMENT( baue_two, 0 );
  LFDS711_BTREE_AU_SET_VALUE_IN_ELEMENT( baue_two, 2 );
  alr = lfds711_btree_au_insert( &baus, &baue_two, NULL );
