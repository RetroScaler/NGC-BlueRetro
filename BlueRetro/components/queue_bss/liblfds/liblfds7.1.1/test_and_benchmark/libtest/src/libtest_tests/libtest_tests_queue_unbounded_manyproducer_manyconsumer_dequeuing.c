...
void libtest_tests_queue_umm_dequeuing( struct lfds711_list_asu_state *list_of_logical_processors, struct libshared_memory_state *ms, enum lfds711_misc_validity *dvs )
{
  ...
  *dvs = LFDS711_MISC_VALIDITY_VALID;
  ...
  {
    LFDS711_QUEUE_UMM_SET_VALUE_IN_ELEMENT( (te_array+loop)->qe, loop );
...
static libshared_pal_thread_return_t LIBSHARED_PAL_THREAD_CALLING_CONVENTION thread_simple_dequeuer( void *libtest_threadset_per_thread_state )
{
  ...
  prev_value = LFDS711_QUEUE_UMM_GET_VALUE_FROM_ELEMENT( *qe );

  libtest_threadset_thread_ready_and_wait( pts );

  while( lfds711_queue_umm_dequeue(tpts->qs, &qe) )
  {
    value = LFDS711_QUEUE_UMM_GET_VALUE_FROM_ELEMENT( *qe );
