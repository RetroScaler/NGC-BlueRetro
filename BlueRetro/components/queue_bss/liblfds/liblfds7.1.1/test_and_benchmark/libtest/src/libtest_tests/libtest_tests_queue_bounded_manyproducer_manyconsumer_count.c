...
void libtest_tests_queue_bmm_count( struct lfds711_list_asu_state *list_of_logical_processors, struct libshared_memory_state *ms, enum lfds711_misc_validity *dvs )
{
  ...
  *dvs = LFDS711_MISC_VALIDITY_VALID;
  ...
  lfds711_queue_bmm_init_valid_on_current_logical_core( &qbmms, qbmme_array, 8, NULL );
  ...
  for( loop = 0 ; loop < 64 ; loop++ )
  {
    ...
    {
      rv = lfds711_queue_bmm_enqueue( &qbmms, NULL, NULL );
    ...
    // TRD : dequeue one
    rv = lfds711_queue_bmm_dequeue( &qbmms, (void **) &key, (void **) &value );
