...
void libtest_tests_queue_umm_alignment( struct lfds711_list_asu_state *list_of_logical_processors, struct libshared_memory_state *ms, enum lfds711_misc_validity *dvs )
{
  ...
  *dvs = LFDS711_MISC_VALIDITY_VALID;
  ...
  // TRD : struct lfds711_queue_umm_element
  if( LIBTEST_MISC_OFFSETOF(struct lfds711_queue_umm_element,next) % LFDS711_PAL_ALIGN_DOUBLE_POINTER != 0 )
    *dvs = LFDS711_MISC_VALIDITY_INVALID_TEST_DATA;



  // TRD : struct lfds711_queue_umm_state
  if( LIBTEST_MISC_OFFSETOF(struct lfds711_queue_umm_state,enqueue) % LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES != 0 )
    *dvs = LFDS711_MISC_VALIDITY_INVALID_TEST_DATA;

  if( LIBTEST_MISC_OFFSETOF(struct lfds711_queue_umm_state,dequeue) % LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES != 0 )
    *dvs = LFDS711_MISC_VALIDITY_INVALID_TEST_DATA;

  if( LIBTEST_MISC_OFFSETOF(struct lfds711_queue_umm_state,aba_counter) % LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES != 0 )
    *dvs = LFDS711_MISC_VALIDITY_INVALID_TEST_DATA;

  if( LIBTEST_MISC_OFFSETOF(struct lfds711_queue_umm_state,user_state) % LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES != 0 )
    *dvs = LFDS711_MISC_VALIDITY_INVALID_TEST_DATA;
