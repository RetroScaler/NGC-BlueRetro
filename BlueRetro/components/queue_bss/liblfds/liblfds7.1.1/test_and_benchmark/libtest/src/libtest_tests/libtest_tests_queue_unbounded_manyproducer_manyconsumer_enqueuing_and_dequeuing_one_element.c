...
void libtest_tests_queue_umm_enqueuing_and_dequeuing_one_element( struct lfds711_list_asu_state *list_of_logical_processors, struct libshared_memory_state *ms, enum lfds711_misc_validity *dvs )
{
  ...
  *dvs = LFDS711_MISC_VALIDITY_VALID;
  ...
  te.thread_number = 0;
  te.counter = 0;
  LFDS711_QUEUE_UMM_SET_VALUE_IN_ELEMENT( te.qe, &te );
  lfds711_queue_umm_enqueue( &qs, &te.qe );
  ...
  vi.min_elements = vi.max_elements = 1;
...
static libshared_pal_thread_return_t LIBSHARED_PAL_THREAD_CALLING_CONVENTION thread_enqueuer_and_dequeuer( void *libtest_threadset_per_thread_state )
{
  ...
  while( current_time < start_time + TEST_DURATION_IN_SECONDS )
    if( lfds711_queue_umm_dequeue(tpts->qs, &qe) )
    {
