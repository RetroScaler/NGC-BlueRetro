...
void libtest_tests_btree_au_alignment( struct lfds711_list_asu_state *list_of_logical_processors, struct libshared_memory_state *ms, enum lfds711_misc_validity *dvs )
{
  ...
  *dvs = LFDS711_MISC_VALIDITY_VALID;
  ...
  // TRD : struct lfds711_btree_au_element
  if( LIBTEST_MISC_OFFSETOF(struct lfds711_btree_au_element,up) % LFDS711_PAL_ALIGN_SINGLE_POINTER != 0 )
    *dvs = LFDS711_MISC_VALIDITY_INVALID_TEST_DATA;

  if( LIBTEST_MISC_OFFSETOF(struct lfds711_btree_au_element,left) % LFDS711_PAL_ALIGN_SINGLE_POINTER != 0 )
    *dvs = LFDS711_MISC_VALIDITY_INVALID_TEST_DATA;

  if( LIBTEST_MISC_OFFSETOF(struct lfds711_btree_au_element,right) % LFDS711_PAL_ALIGN_SINGLE_POINTER != 0 )
    *dvs = LFDS711_MISC_VALIDITY_INVALID_TEST_DATA;

  if( LIBTEST_MISC_OFFSETOF(struct lfds711_btree_au_element,value) % LFDS711_PAL_ALIGN_SINGLE_POINTER != 0 )
    *dvs = LFDS711_MISC_VALIDITY_INVALID_TEST_DATA;



  // TRD : struct lfds711_btree_au_state
  if( LIBTEST_MISC_OFFSETOF(struct lfds711_btree_au_state,root) % LFDS711_PAL_ALIGN_SINGLE_POINTER != 0 )
    *dvs = LFDS711_MISC_VALIDITY_INVALID_TEST_DATA;
