...
void libtest_tests_queue_umm_enqueuing( struct lfds711_list_asu_state *list_of_logical_processors, struct libshared_memory_state *ms, enum lfds711_misc_validity *dvs )
{
  ...
  *dvs = LFDS711_MISC_VALIDITY_VALID;
  ...
  lfds711_queue_umm_init_valid_on_current_logical_core( &qs, &dummy_qe, NULL );
  ...
  */
  ...
  while( *dvs == LFDS711_MISC_VALIDITY_VALID and lfds711_queue_umm_dequeue(&qs, &qe) )
  {
...
static libshared_pal_thread_return_t LIBSHARED_PAL_THREAD_CALLING_CONVENTION thread_simple_enqueuer( void *libtest_threadset_per_thread_state )
{
  ...
  for( loop = 0 ; loop < tpts->number_elements_per_thread ; loop++ )
  {
    LFDS711_QUEUE_UMM_SET_VALUE_IN_ELEMENT( (tpts->te_array+loop)->qe, tpts->te_array+loop );
    lfds711_queue_umm_enqueue( tpts->qs, &(tpts->te_array+loop)->qe );
  }
