...
void libtest_tests_freelist_without_ea_popping_and_pushing( struct lfds711_list_asu_state *list_of_logical_processors, struct libshared_memory_state *ms, enum lfds711_misc_validity *dvs )
{
  ...
  *dvs = LFDS711_MISC_VALIDITY_VALID;
  ...
  while( LFDS711_LIST_ASU_GET_START_AND_THEN_NEXT(*list_of_logical_processors,lasue) )
  {
    ...
    lfds711_freelist_init_valid_on_current_logical_core( &(tpts+loop)->fs_thread_local, NULL, 0, NULL );
    ...
    lfds711_freelist_init_valid_on_current_logical_core( &(tpts+loop+number_logical_processors)->fs_thread_local, NULL, 0, NULL );
    ...
    for( subloop = number_elements_per_thread * (number_logical_processors + loop) ; subloop < number_elements_per_thread * (number_logical_processors + loop + 1) ; subloop++ )
    {
      ...
      lfds711_freelist_push( &(tpts+loop+number_logical_processors)->fs_thread_local, &(te_array+subloop)->thread_local_fe, NULL );
    }
...
static libshared_pal_thread_return_t LIBSHARED_PAL_THREAD_CALLING_CONVENTION thread_popping_and_pushing_start_popping( void *libtest_threadset_per_thread_state )
{
  ...
  while( time(NULL) < start_time + TEST_DURATION_IN_SECONDS )
  {
    ...
    while( count < tpts->number_elements_per_thread )
      if( lfds711_freelist_pop(tpts->fs, &fe, NULL) )
      {
        lfds711_freelist_push( &tpts->fs_thread_local, fe, NULL );
        count++;
      }

    // TRD : return our local freelist to the main freelist
    while( lfds711_freelist_pop(&tpts->fs_thread_local, &fe, NULL) )
