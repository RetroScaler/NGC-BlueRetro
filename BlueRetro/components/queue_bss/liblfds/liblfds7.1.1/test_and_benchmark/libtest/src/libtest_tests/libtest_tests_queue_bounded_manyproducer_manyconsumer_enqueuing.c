...
void libtest_tests_queue_bmm_enqueuing( struct lfds711_list_asu_state *list_of_logical_processors, struct libshared_memory_state *ms, enum lfds711_misc_validity *dvs )
{
  ...
  */
  ...
  while( *dvs == LFDS711_MISC_VALIDITY_VALID and lfds711_queue_bmm_dequeue(&qbmms, (void **) &thread_number, (void **) &counter_number) )
  {
...
static libshared_pal_thread_return_t LIBSHARED_PAL_THREAD_CALLING_CONVENTION thread_simple_enqueuer( void *libtest_threadset_per_thread_state )
{
  ...
  while( lfds711_queue_bmm_enqueue(tpts->qbmms, (void *) (tpts->thread_number), (void *) (counter++)) );
