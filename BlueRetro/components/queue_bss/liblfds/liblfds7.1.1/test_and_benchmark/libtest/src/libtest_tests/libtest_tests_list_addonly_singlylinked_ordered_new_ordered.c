...
void libtest_tests_list_aso_new_ordered( struct lfds711_list_asu_state *list_of_logical_processors, struct libshared_memory_state *ms, enum lfds711_misc_validity *dvs )
{
  ...
  /* TRD : run one thread per logical processor
           ...
           calling lfds711_list_aso_insert_element_by_position( LFDS711_LIST_ASO_POSITION_ORDERED )
  ...
  *dvs = LFDS711_MISC_VALIDITY_VALID;
  ...
  lfds711_list_aso_init_valid_on_current_logical_core( &lasos, new_ordered_compare_function, LFDS711_LIST_ASO_INSERT_RESULT_FAILURE_EXISTING_KEY, NULL );
  ...
  */
  ...
  if( *dvs == LFDS711_MISC_VALIDITY_VALID )
  {
    ...
    while( *dvs == LFDS711_MISC_VALIDITY_VALID and LFDS711_LIST_ASO_GET_START_AND_THEN_NEXT(lasos, lasoe) )
    {
