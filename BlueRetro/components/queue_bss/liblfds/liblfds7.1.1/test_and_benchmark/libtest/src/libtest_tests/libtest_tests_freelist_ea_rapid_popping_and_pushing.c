...
void libtest_tests_freelist_ea_rapid_popping_and_pushing( struct lfds711_list_asu_state *list_of_logical_processors, struct libshared_memory_state *ms, enum lfds711_misc_validity *dvs )
{
  ...
  *dvs = LFDS711_MISC_VALIDITY_VALID;
  ...
  te_array = libshared_memory_alloc_from_unknown_node( ms, sizeof(struct test_element) * (number_logical_processors + ea_size_in_freelist_elements), LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );
  ...
  lfds711_freelist_query( &fs, LFDS711_FREELIST_QUERY_SINGLETHREADED_GET_COUNT, NULL, (void *) &count );
...
static libshared_pal_thread_return_t LIBSHARED_PAL_THREAD_CALLING_CONVENTION thread_rapid_popping_and_pushing( void *libtest_threadset_per_thread_state )
{
  ...
  {
    lfds711_freelist_pop( tpts->fs, &fe, &tpts->psts );
