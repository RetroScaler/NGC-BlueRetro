...
void libtest_tests_queue_bmm_enqueuing_and_dequeuing( struct lfds711_list_asu_state *list_of_logical_processors, struct libshared_memory_state *ms, enum lfds711_misc_validity *dvs )
{
  ...
  */
  ...
  for( loop = 0 ; loop < number_logical_processors ; loop++ )
    lfds711_queue_bmm_enqueue( &qbmms, (void *) loop, (void *) 0 );
...
static libshared_pal_thread_return_t LIBSHARED_PAL_THREAD_CALLING_CONVENTION thread_enqueuer_and_dequeuer( void *libtest_threadset_per_thread_state )
{
  ...
  while( current_time < start_time + TEST_DURATION_IN_SECONDS )
  {
    ...
    */
    ...
    while( 0 == lfds711_queue_bmm_dequeue(tpts->qbmms, (void *) &thread_number, (void *) &counter) );
    ...
    while( 0 == lfds711_queue_bmm_enqueue(tpts->qbmms, (void *) thread_number, (void *) counter) );
