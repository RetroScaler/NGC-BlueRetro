...
void libtest_tests_pal_atomic_cas( struct lfds711_list_asu_state *list_of_logical_processors, struct libshared_memory_state *ms, enum lfds711_misc_validity *dvs )
{
  ...
  */
  ...
  atcs = libshared_memory_alloc_from_unknown_node( ms, sizeof(struct libtest_tests_pal_atomic_cas_state) * number_logical_processors, LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );
...
static libshared_pal_thread_return_t LIBSHARED_PAL_THREAD_CALLING_CONVENTION thread_cas( void *libtest_threadset_per_thread_state )
{
  ...
  lfds711_pal_uint_t LFDS711_PAL_ALIGN(LFDS711_PAL_ALIGN_SINGLE_POINTER)
    exchange,
    compare;
  ...
  while( loop++ < 10000000 )
  {
    ...
    {
      exchange = compare + 1;
      LFDS711_PAL_ATOMIC_CAS( atcs->shared_counter, &compare, exchange, LFDS711_MISC_CAS_STRENGTH_WEAK, result );
    }
    while( result == 0 );
