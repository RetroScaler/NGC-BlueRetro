...
void libtest_tests_hash_a_random_adds_fail_on_existing( struct lfds711_list_asu_state *list_of_logical_processors, struct libshared_memory_state *ms, enum lfds711_misc_validity *dvs )
{
  ...
  *dvs = LFDS711_MISC_VALIDITY_VALID;
  ...
  element_array = libshared_memory_alloc_largest_possible_array_from_unknown_node( ms, sizeof(struct test_element), LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES, &number_elements_total );
  ...
  lfds711_hash_a_init_valid_on_current_logical_core( &has, baus, 1000, key_compare_function, key_hash_function, LFDS711_HASH_A_EXISTING_KEY_FAIL, NULL );
  ...
  /* TRD : now we attempt to lfds711_hash_a_get_by_key() for every element in number_array
           any failure to find is an error
           we also check we've obtained the correct element
  */

  for( loop = 0 ; *dvs == LFDS711_MISC_VALIDITY_VALID and loop < number_elements_total ; loop++ )
    if( 0 == lfds711_hash_a_get_by_key(&has, NULL, NULL, (void *) (tpts->element_array+loop)->key, &hae) )
      *dvs = LFDS711_MISC_VALIDITY_INVALID_MISSING_ELEMENTS;
    else
    {
      value = (lfds711_pal_uint_t) LFDS711_HASH_A_GET_VALUE_FROM_ELEMENT( *hae );
...
static libshared_pal_thread_return_t LIBSHARED_PAL_THREAD_CALLING_CONVENTION thread_adding( void *libtest_threadset_per_thread_state )
{
  ...
  while( index < tpts->number_elements_per_thread )
  {
    ...
    LFDS711_HASH_A_SET_VALUE_IN_ELEMENT( (tpts->element_array+index)->hae, (tpts->element_array+index)->datum );
