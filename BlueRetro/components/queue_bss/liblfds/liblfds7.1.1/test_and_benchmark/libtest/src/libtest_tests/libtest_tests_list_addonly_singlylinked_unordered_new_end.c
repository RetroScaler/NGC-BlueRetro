...
void libtest_tests_list_asu_new_end( struct lfds711_list_asu_state *list_of_logical_processors, struct libshared_memory_state *ms, enum lfds711_misc_validity *dvs )
{
  ...
  /* TRD : run one thread per logical processor
           ...
           each thread loops, calling lfds711_list_asu_new_element_by_position( LFDS711_LIST_ASU_POSITION_END )
...
static libshared_pal_thread_return_t LIBSHARED_PAL_THREAD_CALLING_CONVENTION new_end_thread( void *libtest_threadset_per_thread_state )
{
  ...
  for( loop = 0 ; loop < tpts->number_elements_per_thread ; loop++ )
  {
    ...
    lfds711_list_asu_insert_at_position( tpts->lasus, &(tpts->element_array+loop)->lasue, NULL, LFDS711_LIST_ASU_POSITION_END );
  }
