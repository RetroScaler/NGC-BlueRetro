...
void libtest_tests_freelist_ea_popping_and_pushing( struct lfds711_list_asu_state *list_of_logical_processors, struct libshared_memory_state *ms, enum lfds711_misc_validity *dvs )
{
  ...
  *dvs = LFDS711_MISC_VALIDITY_VALID;
  ...
  ea = libshared_memory_alloc_from_unknown_node( ms, sizeof(struct lfds711_freelist_element **) * (number_logical_processors * 2 + 1), sizeof(struct lfds711_freelist_element *) );
  for( loop = 0 ; loop < number_logical_processors * 2 + 1 ; loop++ )
    ea[loop] = libshared_memory_alloc_from_unknown_node( ms, sizeof(struct lfds711_freelist_element *) * LFDS711_FREELIST_ELIMINATION_ARRAY_ELEMENT_SIZE_IN_FREELIST_ELEMENTS * smallest_power_of_two_larger_than_or_equal_to_number_logical_processors, LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES );
  ...
  lfds711_freelist_init_valid_on_current_logical_core( &fs, ea[0], smallest_power_of_two_larger_than_or_equal_to_number_logical_processors, NULL );
  ...
  while( LFDS711_LIST_ASU_GET_START_AND_THEN_NEXT(*list_of_logical_processors,lasue) )
  {
    ...
    lfds711_freelist_init_valid_on_current_logical_core( &(tpts+loop)->fs_thread_local, ea[loop+1], smallest_power_of_two_larger_than_or_equal_to_number_logical_processors, NULL );
    ...
    LFDS711_PRNG_ST_GENERATE( psts, random_value );
    LFDS711_PRNG_ST_MIXING_FUNCTION( random_value );
    lfds711_prng_st_init( &(tpts+loop+number_logical_processors)->psts, random_value );
    (tpts+loop+number_logical_processors)->number_elements_per_thread = number_elements_per_thread;
    lfds711_freelist_init_valid_on_current_logical_core( &(tpts+loop+number_logical_processors)->fs_thread_local, ea[loop+1+number_logical_processors], number_logical_processors, NULL );
    ...
    {
      LFDS711_FREELIST_SET_VALUE_IN_ELEMENT( (te_array+subloop)->thread_local_fe, (te_array+subloop) );
      lfds711_freelist_push( &(tpts+loop+number_logical_processors)->fs_thread_local, &(te_array+subloop)->thread_local_fe, &psts );
    }
  ...
  lfds711_freelist_query( &fs, LFDS711_FREELIST_QUERY_SINGLETHREADED_VALIDATE, (void *) &vi, (void *) dvs );

  lfds711_freelist_cleanup( &fs, NULL );

  for( loop = 0 ; loop < number_logical_processors ; loop++ )
  {
    lfds711_freelist_cleanup( &(tpts+loop)->fs_thread_local, NULL );
    lfds711_freelist_cleanup( &(tpts+loop+number_logical_processors)->fs_thread_local, NULL );
  }
...
static libshared_pal_thread_return_t LIBSHARED_PAL_THREAD_CALLING_CONVENTION thread_popping_and_pushing_start_popping( void *libtest_threadset_per_thread_state )
{
  ...
  while( time(NULL) < start_time + TEST_DURATION_IN_SECONDS )
  {
    ...
    while( count < tpts->number_elements_per_thread )
      if( lfds711_freelist_pop(tpts->fs, &fe, &tpts->psts) )
      {
        // TRD : we do nothing with the test data, so there'fs no GET or SET here
        lfds711_freelist_push( &tpts->fs_thread_local, fe, &tpts->psts );
        count++;
      }

    // TRD : return our local freelist to the main freelist
    while( lfds711_freelist_pop(&tpts->fs_thread_local, &fe, &tpts->psts) )
      lfds711_freelist_push( tpts->fs, fe, &tpts->psts );
  }
