...
void libtest_tests_queue_bmm_alignment( struct lfds711_list_asu_state *list_of_logical_processors, struct libshared_memory_state *ms, enum lfds711_misc_validity *dvs )
{
  ...
  *dvs = LFDS711_MISC_VALIDITY_VALID;
  ...
  // TRD : struct lfds711_queue_bmm_state
  if( LIBTEST_MISC_OFFSETOF(struct lfds711_queue_bmm_state,read_index) % LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES != 0 )
    *dvs = LFDS711_MISC_VALIDITY_INVALID_TEST_DATA;

  if( LIBTEST_MISC_OFFSETOF(struct lfds711_queue_bmm_state,write_index) % LFDS711_PAL_ATOMIC_ISOLATION_IN_BYTES != 0 )
    *dvs = LFDS711_MISC_VALIDITY_INVALID_TEST_DATA;
