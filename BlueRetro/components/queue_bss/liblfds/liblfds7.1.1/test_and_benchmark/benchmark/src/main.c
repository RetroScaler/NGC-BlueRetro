...
static void memory_cleanup_callback( enum flag known_numa_node_flag, void *store, lfds711_pal_uint_t size_in_bytes );
  ...
  */
  ...
  if( rv == 1 )
  {
    ...
    if( arg_data != NULL )
    {
      ...
      otp->memory_in_megabytes = (lfds711_pal_uint_t) arg_data->integer.integer;
    ...
    if( arg_data != NULL )
    {
      ...
      otp->benchmark_duration_in_seconds = (lfds711_pal_uint_t) arg_data->integer.integer;
    ...
    if( arg_data != NULL )
    {
      ...
      otp->gnuplot_width_in_pixels = (lfds711_pal_uint_t) arg_data->integer.integer;
    ...
    if( arg_data != NULL )
    {
      ...
      otp->gnuplot_height_in_pixels = (lfds711_pal_uint_t) arg_data->integer.integer;
...
static void perform_operations( struct operations_to_perform *otp )
{
  ...
  if( otp->run_flag == RAISED )
  {
    ...
    if( otp->gnuplot_file_flag == RAISED or otp->gnuplot_png_flag == RAISED )
    {
      ...
      while( LFDS711_LIST_ASU_GET_START_AND_THEN_NEXT(list_of_gnuplots,lasue) )
      {
        bg = LFDS711_LIST_ASU_GET_VALUE_FROM_ELEMENT( *lasue );
